/** Job–candidate match: same 100-point rubric as `backend/utils/calculateJobMatch.js` (keep in sync manually). */
...
/** Merge optional CV Keyword Analysis skills into matching (deduped by norm). */
function mergeCvAnalysisSkills(candidate) {
  ...
  const seen = new Set(
    baseArr.map((s) =>
      norm(typeof s === "string" ? s : String(s?.title ?? s?.name ?? ""))
    )
  );
...
function hasRichExperience(candidate, expStrNorm) {
  if (Array.isArray(candidate?.experience) && candidate.experience.length > 0) return true;
  return String(expStrNorm || "").length > 45;
}

/**
 * Clean structure for “Why this match?” UI (keep in sync with backend `calculateJobMatch`).
 */
function buildMatchExplanation({
  ...
  const summary = buildMatchSummarySentence(score, {
    field: fieldDisplay,
    skillsCount: skills.length,
    hasLoc: Boolean(loc),
    hasCvKw: cvKeywords.length > 0,
    insufficientData,
  });
...
function buildMatchSummarySentence(score, { field, skillsCount, hasLoc, hasCvKw, insufficientData }) {
  if (insufficientData && score <= 45) {
    return "We need more profile data to score this match confidently.";
  }
  if (score >= 75) {
    if (field && skillsCount)
      return "Strong match — your field and skills line up well with this role.";
    if (skillsCount >= 3) return "Strong match — several of your skills appear in this posting.";
    if (field) return "Strong match — your background fits the role area.";
    return "Strong match based on overlap between your profile and this job.";
  }
  if (score >= 55) {
    if (field && skillsCount)
      return "Good match — this job fits your field and several of your skills.";
    if (skillsCount) return "Good match — your skills overlap with what this job asks for.";
    if (field || hasLoc || hasCvKw) return "Good match for your profile in a few important areas.";
    return "Good match for your profile.";
  }
  if (score >= 40) {
    return "Fair match — some details overlap; filling out your profile may raise the score.";
  }
  return "Limited overlap — add skills, CV text, and location where this role matters.";
}
...
export function calculateJobMatch(candidate, job) {
  ...
  let skPts = 0;
  let skillHits = 0;
  const skillBlob = `${skillsArr.join(" ")} ${cvText} ${bio}`;
...
/** @returns {number|null} */
export function computeJobMatchPct(viewer, job) {
  if (
    !viewer ||
    String(viewer.role || "").toLowerCase() !== "candidate" ||
    !job
  ) {
    return null;
  }
  const fromApi = job.candidateMatch?.matchScore;
  if (typeof fromApi === "number" && Number.isFinite(fromApi)) {
    return Math.max(0, Math.min(100, Math.round(fromApi)));
  }
  return calculateJobMatch(viewer, job).matchScore;
}

/** @returns {'green'|'blue'|'amber'|'gray'} */
export function matchScoreTier(pct) {
